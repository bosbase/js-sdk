...
```html
<script src="/path/to/dist/bosbase.umd.js"></script>
...
```html
<script type="module">
    import BosBase from '/path/to/dist/bosbase.es.mjs'
...
BosBase Web API supports file upload via `multipart/form-data` requests,
...
The default [`LocalAuthStore`](https://github.com/bosbase/js-sdk/blob/master/src/stores/LocalAuthStore.ts) uses the browser's `LocalStorage` if available, otherwise - will fallback to runtime/memory (aka. on page refresh or service restart you'll have to authenticate again).
...
The SDK comes also with a helper [`AsyncAuthStore`](https://github.com/bosbase/js-sdk/blob/master/src/stores/AsyncAuthStore.ts) that you can use to integrate with any 3rd party async storage implementation (_usually this is needed when working with React Native_):
```js
...
const pb = new BosBase('http://127.0.0.1:8090', store)
```

##### Custom auth store

In some situations it could be easier to create your own custom auth store. For this you can extend [`BaseAuthStore`](https://github.com/bosbase/js-sdk/blob/master/src/stores/BaseAuthStore.ts) and pass the new custom instance as constructor argument to the client:
...
const pb = new BosBase('http://127.0.0.1:8090', new CustomAuthStore());
```

##### Common auth store fields and methods

The default `pb.authStore` extends [`BaseAuthStore`](https://github.com/bosbase/js-sdk/blob/master/src/stores/BaseAuthStore.ts) and has the following public members that you can use:
...
Alternatively, if you don't want to type the generic argument every time you can define a global BosBase type using type assertion:
...
All API services accept an optional `options` argument (usually the last one and of type [`SendOptions`](https://github.com/bosbase/js-sdk/blob/master/src/tools/options.ts)), that can be used to provide:
...
1. Create a new `BosBase` instance for each server-side request
...
All [`BaseAuthStore`](https://github.com/bosbase/js-sdk/blob/master/src/stores/BaseAuthStore.ts) instances have 2 helper methods that
...
One way to integrate with SvelteKit SSR could be to create the BosBase client in a [hook handle](https://kit.svelte.dev/docs/hooks#handle)
...
export async function handle({ event, resolve }) {
    event.locals.pb = new BosBase('http://127.0.0.1:8090');
...
For proper `locals.pb` type detection, you can also add `BosBase` in your your global types definition:
...
To integrate with Astro SSR, you could create the BosBase client in the [Middleware](https://docs.astro.build/en/guides/middleware) and pass it to the Astro components using the `Astro.locals`.
...
export const onRequest = defineMiddleware(async ({ locals, request }: any, next: () => any) => {
    locals.pb = new BosBase('http://127.0.0.1:8090');
...
One way to integrate with Nuxt 3 SSR could be to create the BosBase client in a [nuxt plugin](https://v3.nuxtjs.org/guide/directory-structure/plugins)
...
One way to integrate with Nuxt 2 SSR could be to create the BosBase client in a [nuxt plugin](https://nuxtjs.org/docs/directory-structure/plugins#plugins-directory) and provide it as a helper to the `$root` context:
...
<script>
  export default {
    async asyncData({ $bosbase }) {
      // fetch and return all "example" records...
      const items = await $bosbase.collection('example').getFullList();
...
One way to integrate with Next.js SSR could be to create a custom `BosBase` instance in each of your `getServerSideProps`:

```jsx
import BosBase from 'bosbase';

// you can place this helper in a separate file so that it can be reused
async function initBosBase(req, res) {
  const pb = new BosBase('http://127.0.0.1:8090');
...
export async function getServerSideProps({ req, res }) {
  const pb = await initBosBase(req, res)
...
**To prevent a malicious user or 3rd party script to steal your BosBase auth token, it is recommended to configure a basic CSP for your application (either as `meta` tag or HTTP header).**
...
```js
const pb = new BosBase(baseURL = '/', authStore = LocalAuthStore);
```

### Instance methods

> Each instance method returns the `BosBase` instance allowing chaining.
