...
- Fixed outdated `OAuth2Provider` TS fields ([bosbase/site#110](https://github.com/bosbase/site/pull/110)).


## 0.26.2

- Allow body object without constructor ([#352](https://github.com/bosbase/js-sdk/issues/352)).


## 0.26.1

- Set the `cause` property of `ClientResponseError` to the original thrown error/data for easier debugging ([#349](https://github.com/bosbase/js-sdk/pull/349); thanks @shish).
...
for consistency with how `JSON.stringify` works (see [bosbase#6731](https://github.com/bosbase/bosbase/issues/6731#issuecomment-2812382827)).
...
- Ignore query parameters with `undefined` value ([#330](https://github.com/bosbase/js-sdk/issues/330)).
...
- Added support for assigning `FormData` as body to individual batch requests ([bosbase#6145](https://github.com/bosbase/bosbase/discussions/6145)).
...
- Fixed old `pb.authStore.isAdmin`/`pb.authStore.isAuthRecord` and marked them as deprecated in favour of `pb.authStore.isSuperuser` ([#323](https://github.com/bosbase/js-sdk/issues/323)).
    _Note that with BosBase v0.23.0 superusers are converted to a system auth collection so you can always simply check the value of `pb.authStore.record?.collectionName`._


## 0.22.0

**⚠️ This release introduces some breaking changes and works only with BosBase v0.23.0+.**
...
- Added support for authenticating with OTP (email code):
    ...
    Note that BosBase v0.23.0 comes also with Multi-factor authentication (MFA) support.
...
- Instead of replacing the entire `pb.authStore.record`, on auth record update we now only replace the available returned response record data ([bosbase#5638](https://github.com/bosbase/bosbase/issues/5638)).
...
- Shallow copy the realtime subscribe `options` argument for consistency with the other methods ([#308](https://github.com/bosbase/js-sdk/issues/308)).
...
- Exported `HealthService` types ([#289](https://github.com/bosbase/js-sdk/issues/289)).
...
**⚠️ This release works only with BosBase v0.21.0+ due to changes of how the `multipart/form-data` body is handled.**
...
- Partial and temporary workaround for the auto `application/json` -> `multipart/form-data` request serialization of a `json` field when a `Blob`/`File` is found in the request body ([#274](https://github.com/bosbase/js-sdk/issues/274)).
    ...
    A proper fix for this will be implemented with BosBase v0.21.0 where we'll have support for a special `@jsonPayload` multipart body key, which will allow us to submit mixed `multipart/form-data` content (_kindof similar to the `multipart/mixed` MIME_).


## 0.20.2

- Throw 404 error for `getOne("")` when invoked with empty id ([#271](https://github.com/bosbase/js-sdk/issues/271)).

- Added `@throw {ClientResponseError}` jsdoc annotation to the regular request methods ([#262](https://github.com/bosbase/js-sdk/issues/262)).
...
- Added `expand`, `filter`, `fields`, custom query and headers parameters support for the realtime subscriptions.
    ...
    ```
    _This works only with BosBase v0.20.0+._

- Changes to the logs service methods in relation to the logs generalization in BosBase v0.20.0+:
    ```js
...
- Added `pb.filter(rawExpr, params?)` helper to construct a filter string with placeholder parameters populated from an object.
    ...
    - `number`
    - `boolean`
    - `Date` object (_will be stringified into the format expected by BosBase_)
    - `null`
...
- Added optional generic support for the `RecordService` ([#251](https://github.com/bosbase/js-sdk/issues/251)).
    ...
    interface TypedBosBase extends BosBase {
    ...
    const pb = new BosBase("http://127.0.0.1:8090") as TypedBosBase;
...
- Added support for assigning a `Promise` as `AsyncAuthStore` initial value ([#249](https://github.com/bosbase/js-sdk/issues/249)).
...
- Added `pb.backups.upload(data)` action (_available with BosBase v0.18.0_).
...
- Fixed typo in the deprecation console.warn messages ([#235](https://github.com/bosbase/js-sdk/pull/235); thanks @heloineto).
...
- Added `skipTotal=1` query parameter by default for the `getFirstListItem()` and `getFullList()` requests.
  _Note that this have performance boost only with BosBase v0.17+._

- Added optional `download=1` query parameter to force file urls with `Content-Disposition: attachment` (_supported with BosBase v0.17+_).


## 0.15.3

- Automatically resolve pending realtime connect `Promise`s in case `unsubscribe` is called before
  `subscribe` is being able to complete ([bosbase#2897](https://github.com/bosbase/bosbase/discussions/2897#discussioncomment-6423818)).


## 0.15.2

- Replaced `new URL(...)` with manual url parsing as it is not fully supported in React Native ([bosbase#2484](https://github.com/bosbase/bosbase/discussions/2484#discussioncomment-6114540)).
...
- Cancel any pending subscriptions submit requests on realtime disconnect ([#204](https://github.com/bosbase/js-sdk/issues/204)).


## 0.15.0

- Added `fields` to the optional query parameters for limiting the returned API fields (_available with BosBase v0.16.0_).

- Added `pb.backups` service for the new BosBase backup and restore APIs (_available with BosBase v0.16.0_).

- Updated `pb.settings.testS3(filesystem)` to allow specifying a filesystem to test - `storage` or `backups` (_available with BosBase v0.16.0_).


## 0.14.4

- Removed the legacy aliased `BaseModel.isNew` getter since it conflicts with similarly named record fields ([bosbase#2385](https://github.com/bosbase/bosbase/discussions/2385)).
...
- Added option to specify a generic `send()` return type and defined `SendOptions` type ([#171](https://github.com/bosbase/js-sdk/pull/171); thanks @iamelevich).

- Deprecated `SchemaField.unique` prop since its function is replaced by `Collection.indexes` in the upcoming BosBase v0.14.0 release.


## 0.13.0

- Aliased all `BaseModel` helpers with `$` equivalent to avoid conflicts with the dynamic record props ([#169](https://github.com/bosbase/js-sdk/issues/169)).
  ```js
  isNew      -> $isNew
  load(data) -> $load(data)
  clone()    -> $clone()
...
- Updated `pb.beforeSend` and `pb.afterSend` signatures to allow returning and awaiting an optional `Promise` ([#166](https://github.com/bosbase/js-sdk/pull/166); thanks @Bobby-McBobface).

- Added `Collection.indexes` field for the new collection indexes support in the upcoming BosBase v0.14.0.

- Added `pb.settings.generateAppleClientSecret()` for sending a request to generate Apple OAuth2 client secret in the upcoming BosBase v0.14.0.


## 0.12.1

- Fixed request `multipart/form-data` body check to allow the React Native Android and iOS custom `FormData` implementation as valid `fetch` body ([#2002](https://github.com/bosbase/bosbase/discussions/2002)).


## 0.12.0

- Changed the return type of `pb.beforeSend` hook to allow modifying the request url ([#1930](https://github.com/bosbase/bosbase/discussions/1930)).
  ```js
  // old
...
- Exported the services class definitions to allow being used as argument types ([#153](https://github.com/bosbase/js-sdk/issues/153)).
  ```js
  CrudService
  AdminService
  CollectionService
...
- Added more helpful message for the `ECONNREFUSED ::1` localhost error (related to [#21](https://github.com/bosbase/js-sdk/issues/21)).
...
- Added `pb.health.check()` that checks the health status of the API service (_available in BosBase v0.10.0_)


## 0.8.4

- Added type declarations for the action query parameters ([#102](https://github.com/bosbase/js-sdk/pull/102); thanks @sewera).
  ```js
  BaseQueryParams
  ListQueryParams
  RecordQueryParams
...
- Renamed the declaration file extension from `.d.ts` to `.d.mts` to prevent type resolution issues ([#92](https://github.com/bosbase/js-sdk/issues/92)).
...
> ⚠️ Please note that this release works only with the new BosBase v0.8+ API!
>
...
- Changed the contstructor from `BosBase(url, lang?, store?)` to `BosBase(url, store?, lang?)` (aka. the `lang` option is now last).
